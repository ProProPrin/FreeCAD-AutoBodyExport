...
**履歴の保存先ディレクトリ** が空欄の場合、STEP履歴は `step/old_versions/`、
STL履歴は `stl/old_versions/` に保存します。
...
選択ダイアログでは、ドキュメントごとに出力先を上書きできます。ドキュメント専用の
指定ディレクトリでは、選択したディレクトリをそのまま使用します。
...
現在の `.FCStd` があるディレクトリは `{document_dir}` で指定できます。例えば
`{document_dir}/export` は、作業中ドキュメントの隣の `export` ディレクトリへ
出力します。親ディレクトリ配下の `export` ディレクトリへ出力する場合は
`{document_parent_dir}/export` または `{document_dir}/../export` を指定できます。

### 履歴の保存先ディレクトリ

**履歴の保存先ディレクトリ** を指定すると、最新のSTEP/STL出力先とは別の場所へ
履歴を移動できます。入力欄は1つだけで、その配下に形式ごとのサブディレクトリを
自動作成します。
...
例えば `{document_dir}/export_history` を指定すると、STEP履歴は
`export_history/step/vN/`、STL履歴は `export_history/stl/vN/` に保存します。
解決後の最新出力ルートを使う場合は `{output_dir}` を指定できます。

### 置換と履歴

- 最新の出力は常に通常のファイル名を維持します。
- 置換前の管理ファイルは、次の履歴 `vN/` へ
...
| STEP | 有効 | STEPを出力 |
| STL | 無効 | STLを出力 |
| 出力モード | 各ドキュメントと同じ場所 | 新しいドキュメント設定で使う既定の出力先 |
| ファイル名テンプレート | `{document}_{part}_{target}` | 最新出力のファイル名を生成 |
| 保持する履歴世代数 | `20` | 各形式で保持する履歴 `vN/` の最大数 |
| 履歴の保存先ディレクトリ | 空欄 | 任意の履歴ルート。配下に `step` と `stl` を自動作成 |
...
一覧には既知のCADパス、ドキュメント単位の有効状態、選択対象数、管理ファイル数、
ドキュメント専用の出力先を表示します。
